...
public class ProductBigDataQuestions {
    ...
    public static int q1() {
        ...
        // Example: Product1 contains Chemical1, Chemical2 and Product2 contains Chemical2 and Chemical3
        //          count would be 4 total chemicals
    	int totalChemicals = productList.stream().mapToInt(p -> p.getChemicals().size()).sum();
        return totalChemicals; 
    }
    public static int q2() {
        // QUESTION 2: How many different companies are in the dataset?
    	
    	int totalCompanies = (int) productList.stream().map(product -> product.getCompany()).distinct().count();
        return totalCompanies;
    }
    public static long q3() {
        // QUESTION 3: How many products have 4 or more chemicals?
    	return productList.stream().filter(p -> p.getChemicals().size() >= 4).count();
    ...
    public static String q5(Map<String, List<Product>> companyProductMap) {
    	...
    	Optional<String> os =
    	companyProductMap.entrySet().stream().max( (e1, e2) -> Integer.compare(e1.getValue().size(), e2.getValue().size())).map(Map.Entry::getKey);
    	String result = os.orElse("");
    	return result;
    ...
    public static void q6( Map<String, List<Product>> chemicalProductMap) {
    	...
    	productList.stream().forEach(
    			product -> {
    				product.getChemicals().stream().
    					forEach(chemicalName -> 
    						chemicalProductMap.get(chemicalName).add(product));	
    			}
    			);
    }
    public static String q7() {
    	...
    	Optional<String> os =
    	chemicalProductMap.entrySet().stream().max( (e1, e2) -> Integer.compare(e1.getValue().size(), e2.getValue().size())).map(Map.Entry::getKey);
    	String result = os.orElse("");
    	return result;  
    }
