...
public class HomeworkM11Streams {
	...
	public static void main(String[] args) throws Exception {
		...
		// Print a comma-separated String of all customer IDs for customers that spent >
		// 9000:
		...
		// Print the amount spent by the customer. If there is none, nothing should be
		// printed.
		...
		// Q10: Create a String of the numbers represented in the array, separated by
		// semicolons.
	...
	public static long q1() {
		...
		long count = customerStream.filter(customer -> customer.getState().contains("CA")).count();
	...
	public static List<Customer> q2() {
		// Q2: Create a list of all priority customers in MA.
		List<Customer> priorityCustomerList = customerList.stream()
				.filter(customer -> customer.isPriority() == true && customer.getState().contains("MA"))
				.collect(Collectors.toList());
		return priorityCustomerList;
	}

	public static double q3() {
		// Q3: How much money have all customers spent (combined)?
		double customerSpendingTotal = customerList.stream().mapToDouble(Customer::getAmountSpent).sum();
		return customerSpendingTotal;
	}

	public static double q4() {
		// Q4: How much money have all priority customers spent (combined)?
		double priorityCustomerSpendingTotal = customerList.stream().filter(customer -> customer.isPriority() == true)
				.mapToDouble(Customer::getAmountSpent).sum();
		return priorityCustomerSpendingTotal;
	}

	public static Map<String, Customer> q5() {
		// Q5: Create a map of all WY priority customers (key=id, value=customer)
		Map<String, Customer> WYPriorityCustomers = customerList.stream()
				.filter(customer -> customer.isPriority() == true && customer.getState().contains("WY"))
				.collect(Collectors.toMap(customer -> customer.getId(), Function.identity()));
		return WYPriorityCustomers;
	}

	public static double q6() {
		// Q6: What is the greatest amount of money spent by a NY priority customer?
		Customer maxNyCustomer = customerList.stream()
				.filter(customer -> customer.isPriority() == true && customer.getState().contains("NY"))
				.max((customer1, customer2) -> Double.compare(customer1.getAmountSpent(), customer2.getAmountSpent()))
				.get();
		return maxNyCustomer.getAmountSpent();
	}

	public static String q7() {
		// Q7: Find all customers that spent > 9000.
		String bigSpenders = customerList.stream()
	...
	public static int q9(String[] numWords) {
		// Q9: Find the sum of the numbers represented in an String array.
		return Stream.of(numWords).mapToInt(s -> Integer.parseInt(s)).sum();
	}

	public static String q10(Integer[] nums) {
		// Q10: Create a String of the numbers represented in the array, separated by
		// semicolons.
		return Stream.of(nums).map(i -> Integer.toString(i))
				.collect(Collectors.joining(";"));
	}
	...
	public static void q12(List<String> scrabbleWords) {
		...
		// Note: a method is provided below to convert from char to score.
		// Hint: you might consider writing another method to find the score of a word!
//		Map<String, Integer> highScoringWords = scrabbleWords.stream()
//				.collect(Collectors.toMap(s -> s, wordScore()));
	}
		scrabbleWords.stream().sorted( (word1, word2) -> Integer.compare(wordScore(word2), wordScore(word1))).limit(9).forEach(word -> System.out.println(word + " worth " + wordScore(word) + " points"));
	...
	public static void fillList(List<Customer> list) {
		try (Scanner fileScan = new Scanner(new FileReader(new File("Customers.csv")))) {
