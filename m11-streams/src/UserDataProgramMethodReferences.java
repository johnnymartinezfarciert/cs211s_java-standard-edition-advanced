...
public class UserDataProgramMethodReferences {
	...
	public static void main(String[] args) {
		...
		while(keepGoing) {
			System.out.println("Enter" +
					...
					"\n\t8 to sort by id" +
					"\n\t9 to sort by last name, id" +
					"\n\t10 to exit" );
			int choice = Integer.parseInt(scan.nextLine());

			switch(choice) {
			case 1: Collections.sort(userList, Comparator
					.comparing(User::getFirstName)
					.thenComparing(User::getLastName)); break;
			case 2: Collections.sort(userList, Comparator
					.comparing(User::getLastName)
					.thenComparing(User::getFirstName)); break;
			case 3: Collections.sort(userList, Comparator.comparing(User::getUsername)); break;
			case 4: Collections.sort(userList, Comparator.comparing(User::getEmail)); break;
			case 5: Collections.sort(userList, Comparator
						.comparing( (User user) -> user.getEmail().substring(user.getEmail().indexOf("@")+1))
						.thenComparing(User::getEmail));  break;						
			case 6: Collections.sort(userList, Comparator.comparing(User::getJoinDate)); break;
			case 7: Collections.sort(userList, Comparator.comparing(User::getJoinDate).reversed()); break;
			case 8: Collections.sort(userList, Comparator.comparingInt(User::getId)); break; 
			
			case 9: Collections.sort(userList, Comparator
						.comparing(User::getLastName)
						.thenComparingInt(User::getId)); break;
					
			case 10: System.out.println("Good bye."); keepGoing = false; break;		
